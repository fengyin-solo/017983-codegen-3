...
4. 批量分析队列
   - 点击"添加音频"一次挑选多条音频，或将多个文件拖拽到队列面板
   - 点击"开始分析"后逐条自动分析，每条显示实时进度与状态
   - 整体进度条同步推进，运行期间其他操作按钮自动禁用
   - 单条失败不影响后续任务，失败条目会标红并可单独重试
   - 中途关闭页面再打开，队列进度与已完成的结果自动恢复
   - 点击已完成条目的"查看结果"可将结果加载到图表区，并可另存为记录
...
- **批量分析队列**：
  - 一次挑选多条音频排入队列，逐条自动分析
  - 每条任务实时显示进度与状态（等待中/分析中/已完成/失败）
  - 整体进度条同步推进，运行时自动禁用其他交互，避免重复触发
  - 单条失败不中断后续任务，失败条目可单独重试
  - 队列通过 IndexedDB 持久化，中途关闭页面后进度与结果可恢复
- **可视化图表**：
  - 波形图：显示音频波形
  - 频谱图：显示基频和倍频的强度分布
  - 热力图：显示声强随时间的变化
...
│   │   │   ├── batchQueueManager.js  # 批量分析队列（IndexedDB 持久化）
│   │   │   ├── recordManager.js      # 历史记录管理器
